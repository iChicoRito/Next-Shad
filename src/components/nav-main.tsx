                      ...
                      {item.items?.map((subItem) => (
                        <SidebarMenuSubItem key={subItem.title}>
                          <SidebarMenuSubButton asChild className="cursor-pointer" isActive={pathname === subItem.url}>
                            <Link href={subItem.url} target={item.title === 'Auth Pages' || item.title === 'Errors' ? '_blank' : undefined} rel={item.title === 'Auth Pages' || item.title === 'Errors' ? 'noopener noreferrer' : undefined}>
